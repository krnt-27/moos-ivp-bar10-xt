...
MadgwickAHRS::MadgwickAHRS(float sampleFreq, float beta) : 
    ...
    roll(0.0f), pitch(0.0f), yaw(0.0f), anglesComputed(false) {
    invSampleFreq = 1.0f / sampleFreq;
}

void MadgwickAHRS::updateMARG(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
    float recipNorm;
    float s0, s1, s2, s3;
...
void MadgwickAHRS::computeAngles() {
    roll = std::atan2(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2) * 57.29577951f; // degrees
    pitch = std::asin(-2.0f * (q1*q3 - q0*q2)) * 57.29577951f;
    yaw = std::atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3) * 57.29577951f;
    anglesComputed = true;
}
